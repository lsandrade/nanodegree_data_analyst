...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
          ...
          rdf_schema = line['rdf-schema#label']
          ...
          if '(' in rdf_schema:
            rdf_schema = rdf_schema[:rdf_schema.index('(')]
          ...
          data.append({'rdf_schema': rdf_schema, 'binomialAuthority': binomialAuthority})
...
def update_db(data, db):
    ...
    """
    for row in data:
      db.arachnid.update({"label":row['rdf_schema']},{"$set":{"classification.binomialAuthority":row['binomialAuthority']}})
